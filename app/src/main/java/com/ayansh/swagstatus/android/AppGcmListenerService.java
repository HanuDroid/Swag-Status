...
public class AppGcmListenerService extends HanuFCMMessagingService {
	...
	private void notifyNewContent(ResultObject result) {
		...
		if (postsDownloaded == 0) {
			title = "New Status downloaded.";
		} else {
			title = postsDownloaded + " new Status(es) have been downloaded";
		}
	...
	private void triggerWidgetUpdate() {
		...
		int[] appWidgetIds = widgetManager.getAppWidgetIds(new ComponentName(this, SSAppWidgetProvider.class));
